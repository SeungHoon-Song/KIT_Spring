...
public class LogAdvice {
	...
	//언제 결합해야 되는 지를 알려주어야 하는데, 이를 pointcut(결합이 될 때의 시점)이라고 한다.
	...
	//매개변수가 0개 이상이라는 뜻
	@Before("execution(* com.koreait.service.SampleService*.*(..))")	//*접근자 상관없이 패키지에 클래스안에서 이름 상관없이 (..)어떤 매개변수도 상관없이
	...
	//args(매개변수명, 매개변수명, ...)
	//호출된 종단관심사의 매개변수를 횡단관심사로 전달받을 때에는 매갸변수의 개수와 타입에 맞게
	...
	//pointcut은 횡단관심사와 종단관심사의 결합되는 지점을 결정하는 것이다. (프록시가 언제 결합해줄지)
