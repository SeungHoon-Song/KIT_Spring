...
public class ReplyController {
	...
	//댓글 등록
	//브라우저에서 JSON타입으로 데이터를 전송하고 서버에서는 댓글의 처리 결과에 따라 문자열로 결과를 리턴한다. (응답할땐 JSON이나 XML로)
	...
	//ResponseEntity : 서버의 상태코드, 응답 메세지 등을 담을 수 있는 타입 (Ajax통신때문에 사용)
	//@Controller(.jsp)를 사용할꺼면 @RequestBody를 적용하여 JSON데이터를 ReplyVO타입으로 변환하도록 지정한다.
	...
	//댓글 수정
	//PUT : 자원 전체 수정, 자원 내 모든 필드를 전달해야 함, 일부만 전달한 경우 전달되지 않은 필드는 모두 초기화 처리가 된다.
	...
	public ResponseEntity<Integer> remove(@PathVariable("rno") Long rno){
		log.info("remove rno : " + rno);
		
		return new ResponseEntity<Integer>(service.remove(rno), HttpStatus.OK);
	}
