...
<html>
	...
	<script>
		$(document).ready(function(){
			var uploadResult = $(".uploadResult ul");	//첨부파일 이미지 썸네일, 일반 파일을 대표하는 이미지 목록이 들어갈 ul 태그
			...
			$(".uploadResult").on("click", "span", function(e){
				...
				var type = $(this).data("type");	//이미지라면 image, 일반파일이라면 file
			...
			$("input[type='submit']").on("click", function(e){
				...
				var form = $("form[role='form']");	//주로 form태그 가져 올 때 role사용
				var str = "";
				...
				//form 데이터에 첨부파일 정보를 추가한 후 submit 해준다.
			...
			function showUploadResult(uploadResults){
				...
				$(uploadResults).each(function(i, obj){
					//encodeURIComponent() : 서버로 특정 문자열을 전송할 때 명령어로 인식하는 문자 혹은 인식이 불가능한 문자를 코드번호로 변경시켜주는 함수
